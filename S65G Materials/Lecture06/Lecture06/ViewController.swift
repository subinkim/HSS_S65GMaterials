...
class ViewController: UIViewController, GridViewDataSource, UITextFieldDelegate, EngineDelegate {
    ...
    @IBAction func didEndOnExit(_ textField: UITextField) {
        guard let text = textField.text else {  Engine.sharedInstance.size = 10; return }
        guard let gridSize = Int(text) else { Engine.sharedInstance.size = 10; return }
        guard gridSize >= 10
            && gridSize <= 30 else { Engine.sharedInstance.size = 10; return }
